...
trialData_sample <- read_csv("ihw/tdata_sample.csv", header=TRUE, sep=",")
trialData_sample$CUI %>%
  unlist() %>% 
  paste(collapse =";") %>%
  strsplit(";") %>%
  table() %>%
...
getCUIRelations <-function(STRING,SEARCH_TYP,VERSION,VERBOSE){
  ...
  timeout(5)
  SEARCH_URI <- paste("https://uts-ws.nlm.nih.gov/rest/content/2017AB/CUI/",STRING,"/relations", sep = "")
...
#getCUI("fracture of carpal bone","exact","2017AB", TRUE)
res <- getCUIRelations("C0016644","exact","2017AB", TRUE)
